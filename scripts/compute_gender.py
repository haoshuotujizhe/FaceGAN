"""
Compute gender direction via CLIP zero-shot classification + SVM.
Usage: pip install open-clip-torch && python scripts/compute_gender.py
"""
import sys, os, time
...
print(f'[3/5] Sampling {N} 张 + CLIP 分类 (batch={BATCH}, 共{total_batches}批)')
...
pbar = tqdm(total=N, desc='Sampling', unit='img',
...
for batch_idx, i in enumerate(range(0, N, BATCH)):
    ...
    pbar.update(bs)
    pbar.set_postfix({
        'F%': f'{female_running/current*100:.0f}%',
        'speed': f'{speed:.0f}/s',
        'ETA': f'{eta/60:.0f}m{eta%60:.0f}s' if eta < 3600 else f'{eta/3600:.1f}h'
    })
    ...
    if current % CHECKPOINT_EVERY == 0:
        ...
        tqdm.write(f'     checkpoint @ {current}/{N} (F={female_running/current*100:.1f}%)')
...
print(f'\nCLIP classification done: ♀={n_f} ({n_f/N:.0%})  ♂={n_m} ({n_m/N:.0%})')
print(f'    耗时 {elapsed_total/60:.1f} 分钟 ({elapsed_total/N:.2f}秒/张)')

# ─── 4. SVM 找方向 ───
t3 = time.time()
print(f'\n[4/5] SVM training (LinearSVC, {N}样本 × 512维)...')
from sklearn.svm import LinearSVC
svm = LinearSVC(C=1.0, max_iter=5000, dual=True, class_weight='balanced')
svm.fit(X, y)
acc = svm.score(X, y)
print(f'     SVM accuracy: {acc:.2%}  ({time.time()-t3:.1f}s)')

dir_raw = torch.tensor(svm.coef_[0], dtype=torch.float32).to(device).unsqueeze(0)
dir_raw = dir_raw / dir_raw.norm()
print(f'     Direction norm={dir_raw.norm():.4f}, 有效维度={(dir_raw.abs()>0.01).sum().item()}/512')

# ─── 5. 方向验证 ───
t4 = time.time()
print(f'\n[5/5] Validating direction and saving...')
with torch.no_grad():
    ...
    print(f'     a>0 -> female: {pos_f}/8  |  a<0 -> male: {neg_m}/8')
...
print(f'     Saved weights/gender_dir_official.pt')
...
for i, a in enumerate(np.linspace(-4, 4, 9)):
    ...
    label = 'M' if a < -0.5 else 'F' if a > 0.5 else 'N'
...
if os.path.exists(CACHE_DIR):
    ...
    print(f'Cleaned up checkpoint cache {CACHE_DIR}')

print()
print('='*60)
print(f'All done! Total time { (time.time()-t0)/60:.1f} 分钟')
